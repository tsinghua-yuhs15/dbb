...
*			发送数据。
*******************************************************************************/
void TCPtoUSART(unsigned char* msg,u16 len);
...
void TCPtoUSART(unsigned char *msg,u16 len){
	...
	for(t=0;t<len;t++)
	{
		USART_SendData(USART1, msg[t]);//向串口1发送数据
...
void USARTtoTCP(){
	...
	Write_SOCK_Data_Buffer(0, Tx_Buffer, len);//指定Socket(0~7)发送数据处理,端口0发送23字节数据
	for(t=0;t<len;t++){
		Tx_Buffer[t]=0;
	}
}
...
int main(void)
{
	...
	while (1)
	{
		...
		else if(USART_RX_STA&0x8000)//usart收到数据
		{
			//printf("abcd");
